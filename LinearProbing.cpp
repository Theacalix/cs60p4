        ...
        bool LinearHashTable<HashedObj>::isPrime( int n ) const
        {
            if( n == 2 || n == 3 )
                return true;
         ...
         */
        template <class HashedObj>
        int LinearHashTable<HashedObj>::nextPrime( int n ) const
        {
            if( n % 2 == 0 )
                n++;
         ...
         */
        template <class HashedObj>
        LinearHashTable<HashedObj>::LinearHashTable( const HashedObj & notFound, int size )
         ...
         */
        template <class HashedObj>
        void LinearHashTable<HashedObj>::insert( const HashedObj & key, const HashedObj & obj)
        {
                // Insert x as active
         ...
         */
        template <class HashedObj>
        int LinearHashTable<HashedObj>::findPos( const HashedObj & x ) const
        {
...
/* 7*/      return currentPos;
         ...
         */
        template <class HashedObj>
        void LinearHashTable<HashedObj>::remove( const HashedObj & x )
        {
         ...
         */
        template <class HashedObj>
        const HashedObj & LinearHashTable<HashedObj>::find( const HashedObj & x ) const
        {
            int currentPos = findPos( x );
         ...
         */
        template <class HashedObj>
        bool LinearHashTable<HashedObj>::isActive( int currentPos ) const
        {
         ...
         */
         template <class HashedObj>
        int LinearHashTable<HashedObj>::hash( int key, int tableSize ) const
        {
            if( key < 0 ) key = -key;
